...
from utils.box_utils import py_cpu_nms
import cv2
...
parser.add_argument('-m', '--trained_model', default='./weight_result/model_test.pth',
...
if __name__ == '__main__':
    ...
    # net = MyNet(phase='test', num_cls=num_classes)
    ...
    device = torch.device("cuda" if args.cpu else "cpu")
    net = net.to(device)
    ...
    for i in range(1):
        ...
        img_raw = cv2.resize(img_raw, (img_dim, img_dim), interpolation=cv2.INTER_AREA)
        ...
        tic = time.time()
        print(img.shape)
        conf, loc, landms = net(img)  # forward pass
        print(conf)
        ...
        scores1 = conf.squeeze(0).data.cpu().numpy()
        ...
        # print("--------inds: ", np.where(scores > args.confidence_threshold))
        # print("--------inds1: ", np.where(scores1 > args.confidence_threshold))
        # idxx, checkk = np.where(scores1 > args.confidence_threshold)
        # print(idxx[checkk>0])
